---
title: "Data Wrangling Final Project: Disease Divas"
author: "Afsha Tabasum, Jessica Serrao, Parinitha Kompala, Alisa Tvorun Dunn, Ayush Chakraborty"
date: "11/16/2021"
output:
...
Our last aim, titled the "Bonus Aim" was not initially part of the project scope. We wanted to verify that older age at diagnosis would lead to greater tumors among specific tumor sites where metastates was more common. Specifically we were interested in determining whether bone, as indicated in the 2015 Cell paper, contributed to the greatest amount of metastatic tumors particularly in the older age groups. To study this association, we performed an age stratified analysis based on age groups that were used by a population based study published in the British Journal of Cancer in 2013 [read here](https://www.nature.com/articles/bjc2013268#Tab1).

We also wanted to utilize Natural Language Processing (NLP) in our data sets with the hopes of automating some of the exploratory investigations of text based findings. Ultimately this automation could serve as more efficent methods for pre-processing of exome and transcriptome sequencing data that would faciliate greater downstream analysis of the prostate cancer cohorts. Again, this NLP analysis was outside of the scope of our initial project aims but we were determined to carry out this analysis and thus applied the principles of NLP to the second data set in order to better understand information that was given about cancer stages for each patient. In order to do this, we used Regex and information on cancer stages descriptions found [here.](https://www.cancer.org/cancer/prostate-cancer/detection-diagnosis-staging/staging.html)
...
knitr::include_graphics("~/Desktop/Data-Wrangling-Project/Pipeline_Final.png")
```
...
We used Github, a version-control software readily available on the web, to collaborate with each other throughout the project and efficiently add our individual contributions to our final paper. We created a repository titled "Data Wrangling Project", which included our final Rmd file (this paper), a copy of our project pipeline image, our custom library package, and all relevant data sets throughout our analysis. We followed a standard protocol for adding and updating changes to our files -- we used the git pull command for updating our files and we used the add, commit, and push commands for adding our individual changes to the repository. Utilizing Github allowed all of our group members to work on our individual aims at our own time and pace while keeping everyone else updated on our individual and overall progress. A link to our final repository can be found [here](https://github.com/a-tabs/Data-Wrangling-Project).
...
The link to the data set can be found [here.](https://www.cbioportal.org/study/clinicalData?id=prad_su2c_2015)
...
raw_tc_tab
```
From the table created above, we can see that the mean tumor content seems to be fairly equal among the geographic regions covered in the data. The mean tumor content ranges from 58% in the West Coast/internationally to 63% on the East Coast. Additionally, the standard deviations are also relatively similar across the regions, ranging from 18% to 21%. Notice the sample sizes are disproportionate between geographic region with the Midwest and West Coast containing the greatest number of affected individuals.

Next, we tried to look at the data visually. We decided to make a bar plot to visually assess the distribution of the mean tumor content percentages across sequencing regions. In order to create our plot, we first used the `tidyverse` package to group our data by sequencing region, and then get the mean tumor content levels for each group. We made a new table, `raw_tc_df`, which contains the mean tumor content level by sequencing region that we could feed into `ggplot2` to create our visualization. 

Then, we used `ggplot2` to create our boxplot. Using the data from `raw_tc_df`, we plotted the mean tumor content on the y axis against the sequencing regions on the x axis. We then used several features (`geom_text`, `scale_fill_brewer`, `theme`, just to name a few) to improve the aesthetics of the plot.
```{r}
library(tidyverse)
...
After visually assessing the mean tumor content levels by region, we can see that the tumor content levels do seem to be fairly equal among the geographic regions. Next, we decided to look at the distributions of tumor content levels (using our classification system) by center of sequencing. We made this table using the `gtsummary` package again. We also ran a `Fisher's exact test` and obtained a p-value of 0.9, suggesting that there is not a significant association between Sequencing Site and tumor content level. Again, notice how the sample sizes for the various classifications differ with `Medium` containing the highest quantity of individuals while `Low` contained the fewest. They are relatively similar but the difference from `Medium` to `Low` is 9 individuals. 
```{r}
...
To get a better visual assessment of the tumor content levels by sequencing sites, we made a grouped bar plot using `ggplot2`:
```{r}
...
Our next visualization is a boxplot comparing tumor content by sequencing site. We used the `theme` feature in `ggplot2` to change the fonts and colors in the plot. 
...
Lastly, we plotted the tumor content levels by sequencing site region. We first made a subtable to count the frequencies of tumor content levels by site region, and then fed this table into `ggplot2`. Once again, we utilized the `theme` feature to add labels to each bar and customize the fonts and colors in the graph.
...
Just like in Aim 1.1, we uses the library `dplyr` to perform data manipulation for this Aim. In order to know more about the exposure status, we selected two columns: `Abiraterone..ABI..and.Enzalutamide..ENZA..Exposure.Status` which was a binary column for a patient's exposure to Abiraterone and Enzalutamide (contains Yes's and No's)  and `Prior.Treatment` which stored the information for if the patient had ever received any prior Taxane treatment or not.
...
In order to have few categories for finding the exposure status based on prior treatment found in different sequencing institutes, we needed to club institutes into 4 main regions based on the data we have: `East Coast`, `Midwest`, `West Coast` and `International`. Similar to Aim 1.1, we employed `if-else` code for Aim 1.2.
...
We grouped the Exposure status by Prior Treatment and  added the Sequencing region column into the data frame `aim_1_2`. Next, we calculates the mean of the all `Exposure.Status` counts based on the Sequencing Region and grouped by `Prior.Treatment`
...
The table indicates that the samples are not equally distributed for each region with more being centered among the Midwest and West Coast, as was mentioned previously. Moreover, affected individuals were shown more likely to have not received prior taxane treatment given by the larger quantities shown on the bottom row of the table. Following this numeric summary analysis we  performed  a visual summary of the numbers we acquired above. We created bar graphs with the help of the `ggplot2` package and used `geom_bar` to plot the graph grouped by `Prior.Treatment`. This produced six different graphs for each of the `Center of Sequencing` institutes. Furthermore, to plot the graph, we used the `janitor` and `dplyr` packages to store the tables based on these three variables. 
...
As seen in the graphs and tables, the distribution seems to be not normal and hence we decided to perform a `Fisher's exact test` to calculate the p-value. We see that the p-value is less than 0.001, which means it is statistically significant across all confidence levels and hence there is an significant association between `Sequencing Site` and `Exposure Status`. Similarly we also received a  p-value less than 0.001 for `Prior Treatment status` and `Exposure Status` (using `Pearson's Chi-Squared test`), which means there is a statistically significant association between exposure status/sequencing site and exposure status/prior treatment status.   
...
For Aim 2, we created a mutation count classification system similar to the one made and utilized in Aim 1. To assess how we should divide the counts into "high", "medium", and "low", we first looked at the distribution of Mutation Count:
```{r}
finaldata<-remove_nonuniques
summary(finaldata$Mutation.Count)
```

Based off of the summary above, we divided the mutation count into the following levels of severity:

+**Low Mutation Count**: 0-55 mutations
+**Medium Mutation Count**: 56-110 mutations
+**High Mutation Count**: >110 mutations

Then we added this severity classification to our data frame using our tried and true `if-else` statement and summarized the frequencies of mutation count severity levels:
```{r}
# low = 0 - 55
# med = 56 - 110
# high = >110
...
After we summarized the frequencies of mutation count severity levels, we plotted the frequencies in relation to biopsy sites.

For plotting the severity on a human graph we used the package `gganatogram`, which is a package based on `ggplot2` and allows for better visualization of organs and tissues. Here, we overlaid mutation count data onto anatomical structures from the package. Essentially, we grouped our data set by biopsy site, and then calculated the mean mutation count for each site, and finally used our mutation count classification system to set an appropriate value for visualizing the spread of severity across biopsy sites.

First we constructed a data frame by renaming the biopsy sites exactly as built in the package `gganatogram`. Then, we assigned each tumor site a value corresponding to the severity level that matched the mutation count levels.
...
```
We successfully calculated the average mutation count for each biopsy site! Next, we can  plot these on a human graph. First, note that the ranges for mutation count are much more variable than that of tumor content. Good thing we created a classification system for this! To ensure that our color scale didn't range from 0-1000 mutations, which would cause most of the biopsy sites to be colored the same shade since majority of them would be on the lower end of the scale, we utilized our classification system to normalize our mutation count values to a certain extent. 

In the section below, we created a data frame that is formatted for the `gganatogram` package. It contains two columns -- `organ` and `value`. Organ is the biopsy site, and value is the mutation count (note that the columns must be named like this in order to be fed into `gganatogram`). 

To normalize the mutation count values, we manually checked which level of mutation each relevant biopsy site matched to, and then set that biopsy site's value to the maximum value of that level's range. For example, lymph node's mutation count average was 101, which falls into the medium severity level, which ranges from 56 to 110, so lymph node's value in our data frame would be 110. 

It is also worth noting that we could only plot the biopsy sites which were included in gganatogram's `hgMale_key` data frame due to the way the package works. Hence, we included the package and all it's visually helpful plots with the loss of a couple of sites not included in the library just for the purposes of data wrangling. 
```{r}
...
```
Wow! We just made our first human plot that looks at mutation count severity levels by biopsy site! Thanks, `gganatogram`! As evidenced in the plot, the penis, bone, heart, and stomach biopsy sites seem to have the highest mutation count. This could be useful in discerning which areas the care team should focus on in follow up treatments along with monitoring of cancer progression.  
...
colnames(amc) <- c("Mutation Count", "Fraction Genome Altered", "Tumor Content", 
                   "Exposed")
...
To finish off aim 2, we checked if severity of Mutation Count was associated with frequency of biopsy sites sampled in the first data set. We created a data frame that stored the frequency of each biopsy site from our data set:
```{r}
# Frequencies of biopsy site in first data set
aim_2_3 = data.frame(table(finaldata$Tumor.Site))
aim_2_3
```
Next we utilized the `gganatogram` package once again to plot the frequencies of each biopsy site on a human outline. Note, that since the package is picky about which organs we can use, we had to manually create a data frame with relevant "organs" for the package's plotting function and manually include the values (in this case, the frequency of the biopsy site):
```{r}
...
p2
```
After creating two human plots -- one for mutation count severity by biopsy site, and one for frequency of each biopsy site from our data set, we compared the two side-by-side:
```{r}
...
```
Next we compared severity of tumor content directly next to frequency of biopsy site to  visually assess any possible correlation between the two. Comparing the two side-by-side revealed that there was no association between tumor content severity at a biopsy site and how frequently that biopsy site was sampled in this data set. And with that, we conclude our analysis for Aim 2!
...
Once grouping of the data based on the numeric classifications was complete we merged the text description of the AJCC clinical stage from `classifications` with the `ajcc cols` data frame. This added a sentence description that interpreted the numeric clinical stage. In the output below notice how the sentences are structured.
...
```{r img-with-knitr1, echo=FALSE, fig.align='center', out.width='100%'}
knitr::include_graphics("~/Desktop/Data-Wrangling-Project/MissingValueFlowchart.png")
```
...
```{r img-with-knitr2, echo=FALSE, fig.align='center', out.width='100%'}
knitr::include_graphics("~/Desktop/Data-Wrangling-Project/RelationalDataDiagram.png")
```
...
```{r img-with-knitr3, echo=FALSE, fig.align='center', out.width='100%'}
knitr::include_graphics("~/Desktop/Data-Wrangling-Project/Dashboard.png")
```

# CONCLUSION

## Summary
Prior to 2015, data on tumor biopsies of prostrate cancer was limited due to restrictions in the collection of tumors from affected individuals. Our analysis aimed to understand the associations between individual case presentation and the manifestation of disease with the hopes of facilitating greater downstream analysis of prostate cancer cohorts. In addition we aimed to explore the differences in datasets available from 2015 into 2016 regarding prostate cancer particularly the differences in variables collected and findings observed. We hope our methods of data wrangling employed as part of this project can be added to the growing repository of analytic techniques used in prostate cancer research.

We utilized various patient characteristics to examine their relation to severity of prostate cancer through several data wrangling procedures we learned throughout the quarter. After utilizing three platforms for our cleaning and wrangling processes (R, SQL, and Excel), and creating numerical summaries along with visual representations of our data through R packages such as `gtsummary` and `ggplot2`, we have come to the following conclusions:
  ...
  3. After executing a Kruskal Wallis Rank Sum Test  the data provides no evidence of association between tumor site and age at diagnosis. However as illustrated by the visual summary, as men age, incidence of prostate cancer increases with metastasis of disease occurring more frequently in the lymph nodes and bone. 
...
The first thing we noticed is that these graphs are slightly misleading since we are looking at frequencies instead of proportions. The cBioPortal data set frequencies range from 0-40, while the GDC data set frequencies range from 0 to >200. Therefore, we must make these graphs more comprable. 
...
```
Once we set the proportions and equal y axis ranges, we noticed that the gap between having prior treatment versus no prior treatment was relatively smaller in the patients from the first data set compared to patients from the second data set. Additionally, the proportion of patients who received prior treatment in the first data set seems to be higher across all age groups compared to the second data set. 

This would be a great place to start further analysis if we had more time. We could further investigate  how prior treatment affects diagnosis age, if prior treatment has been linked to later diagnosis age, and where we would be able to find more data to test this association. One thing to note is that the first data set's patients were specifically studied on prior Taxane treatment, while the patients in the second data set did not have a specific treatment mentioned. So, we could also look at Taxane treatment effectiveness in preventing prostate cancer versus other treatments. In addition to investigating diagnosis age's relationship to prior treatment status, we had also noticed that the second data set had a "vital status" column which gives us information on the survival status of the patient. This information was missing from our first data set, so another great next step would be to track the incidence of disease in our second data set (from GDC), and by using our vital status column, we would calculate the patient mortality ratio for the GDC data set. 


## Limitations
Though we were able to derive several conclusions from our data sets, there were a few limitations we faced. For starters, the data set we got from cBioPortal did not have well-defined variables or information on certain variables were calculated, what the units of measurements were, etc. For example, we initially struggled to find information on how tumor content and mutation count were measured, what the units of measurements were, and what ranges of each measurement were considered significant. During our investigation for background research, search engines such as Google weren't helpful. This leads us to our second limitation: prostate cancer data is very niche, and there were not many resources readily available for us to use to get a better understanding on some of the variables we were dealing with. Nevertheless, after reading through the data set's attached paper for understanding tumor content and looking at other academic papers to better understand how mutation count works, we were able to successfully interpret our variables of interest.

Additionally, for the GDC portal we did not have equal sample sizes among the age groups. In fact most of the affected individuals fell within the "60-69" and "70-79" age group indicating a skew in the distribution. More data on individuals older than 79 years old would be helpful in studying the incidence of disease and the prevalence of tumors across different sites. 

## Looking Forward - Clinical Implications
If we were to use our analysis to make conclusions about the clinical implications of the prostate cancer data we analyzed throughout this project, our first piece of advice would be to further study associations between sequencing site and exposure status, along with prior treatment status and exposure status since we obtained significant results when studying these associations. Perhaps these findings could relate to geographic or environmental components causing a higher incidence of prostate cancer in these sequencing site regions. Additionally, it would be worth looking into how age plays into certain factors such as prior treatment status and tumor site. Average age of diagnosis with prostate cancer is 66. One would thus expect the "60 to 69" age group to demonstrate the highest frequency of tumors however, our data suggest the "70 to 79" year olds are demonstrating the greatest amount of tumors. We would therefore suggest from our findings that affected individuals falling within the "70 to 79" age group should be monitored more closely for prostate cancer. Additionally tumor sites such as lymph nodes and liver should be monitored along with bone as they to seem to present with greater metastases. 
